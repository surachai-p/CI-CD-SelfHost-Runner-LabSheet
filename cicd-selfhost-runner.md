...
Pull-based Model คือรูปแบบที่ตัว Runner (เครื่องเซิร์ฟเวอร์ของเรา) เป็นฝ่ายเริ่มต้นการเชื่อมต่อออกไปยัง GitHub/GitLab เอง เพื่อคอยตรวจสอบว่ามีงาน (Jobs) ใหม่ๆ ให้ทำหรือไม่ แทนที่จะรอให้ GitHub ส่งข้อมูลเข้ามา

ข้อดี
- ความง่ายในการตั้งค่าเครือข่าย: ไม่ต้องตั้งค่า Port Forwarding หรือเปิดช่องทางรับข้อมูลขาเข้า (Inbound) บน Firewall

- ทำงานหลัง NAT/VPN ได้: แม้เซิร์ฟเวอร์จะอยู่ในเครือข่ายภายในที่ไม่มี Public IP ก็สามารถทำงานได้ขอแค่เพียงออกอินเทอร์เน็ตได้

- ความเสี่ยงต่ำ: ลดช่องโหว่จากการถูกสแกนพอร์ตจากภายนอก
...
สาเหตุหลักคือเรื่อง "ทิศทางของการเข้าถึงข้อมูล" (Direction of Access):

1. ไม่ต้องเปิดพอร์ตขาเข้า: Push-based ต้องการการเปิดพอร์ต (เช่น SSH หรือ Webhook) เพื่อให้ระบบภายนอกสั่งงานเข้ามา ซึ่งเสี่ยงต่อการถูกโจมตีแบบ Brute Force หรือใช้ช่องโหว่ของโปรโตคอล

2. ไม่ต้องฝากกุญแจไว้ภายนอก: ในระบบ Push-based เรามักต้องนำ SSH Key หรือ Credentials ของเซิร์ฟเวอร์ไปฝากไว้ที่ CI Provider (เช่น GitHub Secrets) แต่ Pull-based จะเก็บ Token ไว้ที่ตัวเครื่องเราเอง

3. ควบคุมจากภายใน: เราสามารถควบคุมได้ว่าเครื่อง Runner จะออกไปคุยกับใคร แทนที่จะปล่อยให้ใครก็ได้ลองเชื่อมต่อเข้ามาที่เครื่องเรา
...
การใช้ npm ci (Clean Install) ออกแบบมาเพื่อระบบ CI/CD และ Production โดยเฉพาะ เนื่องจาก:

- ความแม่นยำ (Strictness): มันจะติดตั้ง Library ตามเวอร์ชันที่ระบุใน package-lock.json แบบเป๊ะๆ 100% หากไฟล์ lock ไม่ตรงกับ package.json มันจะหยุดทำงานทันที (ต่างจาก npm install ที่อาจจะแอบอัปเดตเวอร์ชันให้ใหม่ขึ้นตามเครื่องหมาย ^ หรือ ~)

- ความสะอาด: มันจะลบ Folder node_modules ทิ้งก่อนเริ่มติดตั้งใหม่เสมอ เพื่อป้องกันไฟล์ตกค้างจาก Build ก่อนหน้า

- ความเร็ว: โดยปกติจะทำงานได้เร็วกว่า npm install เพราะข้ามขั้นตอนการวิเคราะห์ Dependency ไปใช้ข้อมูลจาก Lockfile ได้เลย
...
- การรัน Code แปลกปลอม: ใครก็ได้บนโลกสามารถ Fork โปรเจกต์ Public ของคุณ แล้วสร้าง Pull Request ที่มี Workflow แก้ไขเพื่อสั่งให้ Runner รันสคริปต์อันตราย

- การเข้าถึงเครือข่ายภายใน: เนื่องจาก Runner รันอยู่ในเซิร์ฟเวอร์ของคุณ มันจึงมีสิทธิ์เข้าถึงทรัพยากรอื่นๆ ในเครือข่าย (Internal Network) เช่น Database หรือเครื่องอื่นๆ ได้

- การขโมย Secrets: ผู้โจมตีสามารถเขียนสคริปต์ให้ Runner พิมพ์ค่า Environment Variables หรือไฟล์สำคัญออกมาดูได้ง่ายๆ
...
Nginx คือ Software สำหรับเป็น Web Server, Load Balancer และ Reverse Proxy ที่เน้นความเร็วและรองรับผู้ใช้จำนวนมาก

ความสำคัญของการทำ Reverse Proxy
- Security: ช่วยซ่อน IP จริงและโครงสร้างภายในของ Application Server ไว้หลัง Nginx

- SSL Termination: ใช้ Nginx จัดการใบรับรอง HTTPS ในที่เดียว แทนที่จะต้องไปตั้งค่าในทุกๆ Service

- Load Balancing: ช่วยกระจาย Request ไปยังเซิร์ฟเวอร์หลายๆ เครื่อง (Upstream) เพื่อไม่ให้เครื่องใดเครื่องหนึ่งทำงานหนักเกินไป

- Caching & Compression: ช่วยบีบอัดไฟล์หรือเก็บ Cache หน้าเว็บ ทำให้ผู้ใช้เปิดเว็บได้เร็วขึ้นและลดภาระของ App Server
