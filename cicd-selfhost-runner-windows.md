...
- ลองหาสาเหตุแล้วแก้ตามทุกขั้นตอนเลยครับ แต่ก็ไม่ได้เช่นเดิมครับ ลบแล้วสร้างใหม่หลายรอบมากเลยครับอาจารย์ เครียดมากเลยครับ
---
...
- 1. Pull-based Model ของ Self-Hosted Runner คืออะไร และมีข้อดีอย่างไร
คำตอบ: Pull-based Model คือสถาปัตยกรรมที่ตัว Runner (เครื่องแม่ข่ายของผู้ใช้) ทำหน้าที่เป็นฝ่ายเริ่มต้นการติดต่อสื่อสารไปยัง GitHub Actions เพื่อตรวจสอบและดึงงาน (Jobs) มาประมวลผลด้วยตนเองผ่านช่องทาง HTTPS
ข้อดี: ช่วยให้การตั้งค่าระบบเครือข่ายทำได้ง่ายและปลอดภัย เนื่องจากไม่จำเป็นต้องเปิดพอร์ตขาเข้า (Inbound Ports) หรือทำ Port Forwarding บน Firewall
การจัดการ: ผู้ดูแลระบบสามารถควบคุมสภาพแวดล้อมภายในได้ทั้งหมด และลดภาระในการจัดการความปลอดภัยของเครือข่ายภายนอก
...
เขียนคำตอบลงในช่องนี้
- รูปแบบ Pull-based มีความปลอดภัยสูงกว่าเนื่องจากเป็นการสื่อสารทางเดียวจากภายในสู่ภายนอก (Outbound Only)
การลดช่องโหว่: ต่างจาก Push-based ที่ต้องเปิดพอร์ตรับข้อมูลจากอินเทอร์เน็ต ซึ่งเสี่ยงต่อการถูกโจมตีแบบสแกนพอร์ตหรือการเจาะระบบจากภายนอก
ความลับของข้อมูล: IP Address ของเครื่อง Runner จะไม่ถูกเปิดเผยต่อสาธารณะ ทำให้ยากต่อการระบุเป้าหมายในการโจมตีจากผู้ไม่หวังดี
</details>
...
เขียนคำตอบลงในช่องนี้
- npm ci (Clean Install) ถูกออกแบบมาเพื่อใช้ในระบบ CI/CD โดยเฉพาะ เพื่อให้เกิดความแม่นยำและเสถียรภาพในการติดตั้ง Library
ความแม่นยำของเวอร์ชัน: ระบบจะติดตั้ง Package ตามที่ระบุไว้ในไฟล์ package-lock.json อย่างเคร่งครัดเท่านั้น เพื่อป้องกันปัญหา "Version Mismatch" ที่อาจเกิดจากการอัปเดตอัตโนมัติของ npm install
ความสะอาดของระบบ: คำสั่งนี้จะทำการลบโฟลเดอร์ node_modules เดิมทิ้งก่อนเสมอ เพื่อให้มั่นใจว่าไม่มีไฟล์ค้างเก่าที่อาจส่งผลกระทบต่อการทำงานของแอปพลิเคชัน
</details>
...
เขียนคำตอบลงในช่องนี้
- การใช้ Self-Hosted Runner กับ Public Repository ถือเป็นความเสี่ยงด้านความปลอดภัยอย่างร้ายแรง (Significant Security Risk)
การประมวลผลโค้ดอันตราย: ผู้ไม่หวังดีสามารถส่ง Pull Request ที่มีสคริปต์อันตราย (Malicious Code) ในไฟล์ Workflow มาให้เครื่อง Runner ของเราประมวลผลได้โดยตรง
ความเสียหายต่อระบบ: อาจนำไปสู่การถูกขโมยข้อมูลสำคัญ (Secrets), การเข้าถึงเครือข่ายภายในองค์กร หรือการนำทรัพยากรเครื่องไปใช้ในทางที่ผิด เช่น การขุดเงินดิจิทัล
</details>
...
เขียนคำตอบลงในช่องนี้
- Nginx คือซอฟต์แวร์ประสิทธิภาพสูงที่ทำหน้าที่เป็น Web Server, Load Balancer และ Reverse Proxy
ความสำคัญของ Reverse Proxy: ทำหน้าที่เป็นตัวกลางรับคำขอ (Request) จากผู้ใช้ แล้วกระจายงานไปยังแอปพลิเคชันเซิร์ฟเวอร์ (เช่น Node.js) ที่รันอยู่ภายใน
ประโยชน์หลัก: ช่วยเพิ่มระดับความปลอดภัยโดยการซ่อนโครงสร้างพอร์ตภายใน, รองรับการจัดการ SSL/TLS ในจุดเดียว และช่วยเพิ่มประสิทธิภาพในการรับโหลดผ่านการทำ Caching หรือ Load Balancing
</details>
...
เขียนคำตอบลงในช่องนี้
- สถาปัตยกรรมของ Shell และคำสั่ง: ระบบ Linux รองรับการทำงานด้วย Bash Script ซึ่งเป็นมาตรฐานหลักของระบบ CI/CD ส่วนใหญ่ ทำให้การรันสคริปต์ใน Workflow มีความเสถียรและยืดหยุ่นสูง ในขณะที่ Windows จะใช้ PowerShell หรือ CMD เป็นหลัก ซึ่งบ่อยครั้งมักประสบปัญหาเรื่อง Path ของไฟล์หรือไวยากรณ์ (Syntax) ที่ไม่รองรับกับคำสั่งมาตรฐานที่เป็น Unix-based

ประสิทธิภาพในการจัดการทรัพยากรและ Docker: การรัน Docker บน Linux สามารถทำงานผ่าน Docker Engine ได้โดยตรง (Native) ทำให้ประมวลผลได้รวดเร็วและใช้ทรัพยากรน้อยกว่า ส่วนใน Windows จำเป็นต้องทำงานผ่าน Docker Desktop หรือ WSL ซึ่งใช้ทรัพยากรเครื่อง (RAM/CPU) สูงกว่ามากในการจำลองสภาพแวดล้อมเพื่อรัน Container

ความเสถียรและการจัดการสิทธิ์ (Permissions): ระบบ Linux มีการจัดการสิทธิ์ระดับไฟล์และ User (Sudo/Root) ที่ชัดเจนและเป็นระเบียบมากกว่าสำหรับการทำ Automation ส่วนการรันบน Windows มักจะติดปัญหาเรื่องการเข้าถึงไฟล์ที่ถูกล็อกไว้โดยระบบ (File Locking) หรือปัญหาเรื่องการตั้งค่าสิทธิ์ในการติดตั้ง Service ที่มีความซับซ้อนและมีโอกาสเกิดข้อผิดพลาดได้ง่ายกว่าในระยะยาวครับ
</details>
